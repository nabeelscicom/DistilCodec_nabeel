...
class BaselineLightningModule(pl.LightningModule):
    ...
    def construct_model(self):
        self.feature_extractors = nn.ModuleList(
            [load_ssl_model(ckpt_path="/cognitive_comp/wangrui/data/utmos/wav2vec_small.pt"), DomainEmbedding(3, 128),]
        )
...
def compute_utmos(real2gen_pathes: list):
    ...
    real_utmos = []
    gen_utmos = []
    for real_path, gen_path in tqdm(real2gen_pathes, desc='Calculating UTMOS'):
        try:
            real_audio = load_audio(real_path, sampling_rate=sr_16K).to(torch.device('cuda', 0))
            utmos_score_t = utmos.score(wavs=real_audio).tolist()[0]
            real_utmos.append(utmos_score_t)

            gen_audio = load_audio(gen_path, sampling_rate=sr_16K).to(torch.device('cuda', 0))
            utmos_score_t = utmos.score(wavs=gen_audio).tolist()[0]
            gen_utmos.append(utmos_score_t)
        except: 
            print(f'Data error: {gen_path}')
            continue
    
    real_utmos = [num for num in real_utmos if not math.isnan(num)]
    mean_real_utmos = sum(real_utmos) / len(real_utmos)
    gen_utmos = [num for num in gen_utmos if not math.isnan(num)]
    mean_gen_utmos = sum(gen_utmos) / len(gen_utmos)
    utmos_scores = {
        'ref': mean_real_utmos,
        'gen': mean_gen_utmos
    }
